...
Product spec: `docs/FEATURE-SPEC.md`. Deployment: `docs/DEPLOY.md`. Usage, limits and what breaks first: `docs/CAPACITY.md`.
...
npm test               # backend (294) + frontend (105) + admin (37) tests
...
- **A Firestore failure at boot: production refuses to start; development falls back to the JSON file.** In the container that file does not exist, so the fallback used to mean an empty marketplace whose every write died with the instance — now `initStore()` throws when `NODE_ENV=production`, the process exits, Cloud Run retries, and a failed start during a deploy leaves traffic on the old revision. Down is honest; empty loses data. In development the fallback stays (an expired gcloud login should not stop work), and reads and writes track the same `firestoreLive` flag so they can never disagree.
- **A change counts as saved only once Firestore has accepted it.** `persistDiff()` moves its `persisted` snapshot forward per batch, *after* that batch commits; it used to mark everything first, so a failed commit was never resent and the change was lost at the next start. A failed persist is retried every 60 s (`RETRY_MS` in `store.ts`) until one lands, the log says how many changes are in memory only, and a shutdown that still cannot save says they are LOST. `flush()` runs one persist at a time and resolves `true`/`false`, so awaiting it waits for a run that started after the call. `backend/tests/persist-retry.test.ts` holds this.
...
- **Firestore is on the Firebase Spark (free) plan**, whose daily limits are hard stops, not bills: 50,000 reads, 20,000 writes, 20,000 deletes, reset around midnight Pacific (about 12:30–13:30 IST). **Every server start reads every document once**, so documents × starts per day must stay under 50,000 — the Firebase console's data viewer and any script that opens the database spend the same quota. A start that hits the limit now refuses to start (above), so the API is **down until the reset** rather than up and empty; writes past the limit wait in memory and are retried. `docs/CAPACITY.md` has the arithmetic. The count is watched in two places, both built on `documentCount()` / `startsWithinFreeReads()` in `firestore.ts`: the boot line (`summarise()`, every collection in `COLLECTIONS` plus the total, and a warning at `STARTS_WARNING` or fewer starts a day) and two tiles on the admin home screen (`databaseDocuments`, `startsWithinFreeReads` on `/admin/stats`). A new collection goes into `COLLECTIONS` and is counted by both without further change; `backend/tests/read-budget.test.ts` fails if one is left out.
...
- **Opening the app resumes her role.** `LandingOrHome` in `App.tsx` sends a signed-in seller or customer from `/` to `homeFor(role)` on the **first screen of a fresh start only** (`appOpened`, set once the app is up), with `replace` so Back leaves the app rather than bouncing off a door. Reaching `/` from *inside* the app — a back press out of `/seller`, the brand mark — still shows the landing page: a plain redirect on `/` is what was removed when "carry on to your shop" was added, and it made a back press land on a page that threw her somewhere else.
...
- **The row is read top to bottom** (`.notif` in `theme.css`): a tone-tinted status mark, then the product and the time on one line, the sentence for her side under it, then who and which order, then the state pill with the amount opposite. The old row packed the pill inside the title and ran the rest together as `name · SMB5013 · 8/9/2026 11:01 pm` — four facts as one string. Unread rows also carry a tint, under the नवीन heading that already names them; colour is never the only signal.
- **It forgets, on purpose.** `visibleFeed()` keeps a row she has read for `FEED_DAYS` (7) and one she has not for `UNREAD_DAYS` (30) — the seller who was away still finds what she missed, and a delivered order from a fortnight ago stops repeating her orders screen. A row marked `standing` is exempt from both: "your shop is paused" is not something that happened on a Tuesday, it is true until she renews. `unreadCount()` counts over the same window, or the badge promises news the screen has already dropped.
- **Two groups, split on the mark from BEFORE this visit.** `Notifications.tsx` takes `lastSeen()` into state on arrival, because the screen marks everything read a moment later — read it again and every row lands under "earlier". Headings appear only when both groups have rows.
- **Relative days, not timestamps.** `whenKey()` → आज / काल / *n* दिवसांपूर्वी, and a plain date past a week, where counting stops being readable. `daysAgo()` counts calendar days, so 11pm and 1am are a day apart.
...
- **A timer that calls the API skips its tick while `document.visibilityState` is not `'visible'`**, and asks once on `visibilitychange` when she comes back. With CPU always allocated, Cloud Run bills every second an instance is up, and one hidden tab polling once a minute keeps it up all night — the admin console's stats poll (`Shell.tsx`) did exactly that, and the seller's payment-waiting screen (`Subscription.tsx`) could do it for the day an approval takes. `RateOrderGate` follows the same rule.
...
**The container is the root `Dockerfile`**, built with the repository root as context (the backend compiles `../shared/src`); deploy it with `gcloud run deploy shantai-api --source .` from the root. `docs/DEPLOY.md` §1 has the detail. **The backend `build` is `tsc` then `scripts/fix-shared-imports.js`, and the second step is load-bearing:** `tsc` leaves `@shared/*` specifiers in its output untouched, no package of that name exists at runtime, and without the rewrite the image builds cleanly and dies on boot with `ERR_MODULE_NOT_FOUND`. `tsx` reads `paths` itself, so dev, tests and the CLI scripts never notice — only `node dist/…` does. Anything new the compiled server needs at runtime has to be copied into the second stage of the Dockerfile, which takes `backend/dist` and nothing else.

Each deploy stores a new image in Artifact Registry (`cloud-run-source-deploy`), and `artifact-cleanup.json` at the root keeps only the **five newest** — so a Cloud Run rollback reaches five deploys back and no further. The file does nothing until applied with `gcloud artifacts repositories set-cleanup-policies` (`docs/DEPLOY.md` §1, *Old images*).
